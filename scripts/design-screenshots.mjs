...
async function main() {
  ...
  const screens = [
    ...
    // The Potting Shed: the card with its kinds and suggestions, a kind picked, a suggestion taken,
    // the describe-it way in, and a helper's details.
    { name: "shed", path: `/shed/${busy.id}`, full: true },
    {
      ...
      full: true,
      act: async (page) => {
        await page.getByRole("button", { name: /^checker/i }).first().click();
    ...
    },
    {
      ...
      full: true,
      act: async (page) => {
        await page.getByRole("tab", { name: /describe it/i }).click();
        await page.getByLabel(/describe your helper/i).fill("Check the checkout still works before anything is called finished, and never touch payments without asking");
    ...
    ...(shop ? [{ name: "shed-checked", path: `/shed/${shop.id}`, full: true, act: async (page) => { await page.getByRole("button", { name: /^details$/i }).first().click(); await page.waitForTimeout(700); } }] : []),
