  ...
  return (
    <main className="room">
      <div className="room-header">
        ...
        <div>
          <strong>{project.name}</strong> · Parts of your app
        </div>
      </div>

      <div className="settings-intro">
        <p>
          These are the parts of your app as Glasshouse understands them. Every tile says “Working in <em>Login</em>” using these names, so rename anything that is
          not how you would say it. Your changes stick, even when the map is refreshed.
        </p>
        <p className="muted">
      ...
      {!map || map.areas.length === 0 ? (
        <div className="empty">
          <h2>No parts yet.</h2>
          <p>Connect the project with the connector, or run <code>glasshouse map</code> inside the folder, and this page fills in.</p>
        </div>
      ) : (
        <ul className="area-list">
          {map.areas.map((a) => (
            <AreaRow key={a.id} area={a} others={map.areas.filter((o) => o.id !== a.id)} busy={busy !== null} showFolders={showFolders} onRename={(name, description) => send({ action: "rename", id: a.id, name, description }, a.id)} onMerge={(into) => send({ action: "merge", from: a.id, into }, a.id)} />
          ))}
        </ul>
      )}
    </main>
    ...
    <li className={`area-row${area.sensitive ? " sensitive" : ""}`}>
      <div className="area-main">
        {editing ? (
          ...
          >
            <input value={name} onChange={(e) => setName(e.target.value)} maxLength={80} placeholder="Name, in your words" autoFocus />
            <input value={description} onChange={(e) => setDescription(e.target.value)} maxLength={300} placeholder="One line on what this part does" />
          ...
          <>
            <div className="area-name">
              {area.name}
              {area.userCorrected && <span className="tag">your name</span>}
              {area.sensitive && <span className="tag warn">counts as high risk when changed</span>}
            </div>
            <div className="muted">{area.description || "No description yet."}</div>
            {showFolders && <div className="mono muted small">{area.prefixes.map((p) => (p === "." ? "(top-level files)" : p)).join(" · ")}</div>}
          </>
        )}
      </div>
      {!editing && (
        <div className="area-actions">
          ...
          {others.length > 0 && (
            <span className="merge">
              <select value={mergeInto} onChange={(e) => setMergeInto(e.target.value)} disabled={busy}>
