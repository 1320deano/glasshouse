...
export function Tile({ session, project, now, quiet = false }: { session: SessionView; project: string; now: number; quiet?: boolean }) {
  ...
  const headline = task?.headline ?? last?.plain ?? "Starting up";
  const depth = DEPTH_LABELS[session.depth];
  const risk = task?.risk;

  return (
    <section className={`tile${quiet ? " quiet" : ""}${open ? " open" : ""}`}>
      ...
      <div className="location">{task?.location ? <>Working in <span>{task.location}</span></> : "Not in any part of the app yet"}</div>
