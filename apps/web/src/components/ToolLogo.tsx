 ...
 * room.
 *
 * These are the makers' own marks, not drawings of them: the maker's own path and the maker's own
 * viewBox, never redrawn by hand. Colour is the one thing the Room now sets for itself. The marks
 * are all drawn in black, because the palette holds no orange and a single coral burst beside two
 * black marks was the last orange left on the screen. A mark that is re-tinted a *different hue*
 * would be the wrong logo, so no mark is ever given a colour - it is only ever taken down to the
 * ink everything else is written in. The Room's own colour coding for the three tools lives in
 * `TOOL_COLOURS` (labels.ts) instead.
 *
 *   Claude Code  https://claude.ai/favicon.svg          - the Claude burst. Anthropic publishes it
 *                                                         in #D97757; drawn here in black
 ...
 *
 * They must not be redrawn by hand. To refresh one, take the maker's published SVG and use its `d`
 * path, its `fill-rule` and a viewBox that frames that path; leave `fill` below at black.
 *
