...
export function ReportProblem({ projectId }: { projectId?: string }) {
  ...
  if (state === "sent")
    return (
      <span role="status">Thanks, noted.</span>
    );
  if (!open)
    return (
  ...
  return (
    ...
    >
      ...
      <textarea id="problem-note" className="field" value={note} onChange={(e) => setNote(e.target.value)} placeholder="What did you expect, and what did you see instead?" rows={3} maxLength={2000} />
