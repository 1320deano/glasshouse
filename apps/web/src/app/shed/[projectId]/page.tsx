...
export default async function ShedPage({ params, searchParams }: { params: Promise<{ projectId: string }>; searchParams: Promise<{ grow?: string }> }) {
  const { projectId } = await params;
  const { grow } = await searchParams;
  ...
  return <Shed initial={initial} mode={store.mode} shedName={SHED_NAME} siteName={SITE_NAME} helpersCommand={HELPERS_COMMAND} viewer={{ email: viewer.email, admin: viewer.admin, local: viewer.local }} projects={projects} startWith={grow} />;
}
