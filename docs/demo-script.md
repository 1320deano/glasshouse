...
| 5–12 | "Update(src/auth/session.ts)" | Headline flips: "Changing how logged-in users are identified" · Location **Login** · badge flips to **High risk** · the "This is the moment" callout appears on its warm grey block: "Claude Code changed Login. You asked: …" | The first edit in `src/auth`. The callout is automatic on a fresh browser (clear the dismissal: `localStorage.removeItem("glasshouse.walkthrough.<project id>")`). |
